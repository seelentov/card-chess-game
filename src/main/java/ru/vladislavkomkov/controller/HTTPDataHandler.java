...
public class HTTPDataHandler implements AutoCloseable
{
  ...
  {
    context.status(201);
    context.json(games.values().stream().filter(game -> game.getState() == Game.State.LOBBY).map(game -> Map.of("UUID", game.getUUID(),
  ...
  void getGameByKey(Context context)
  {
    ...
    {
      throw new NotFoundResponse("Game already started");
    }
